...
def query():
    if request.method == 'GET':
        ...
        length = []
        has_next = 1
        results=JiangHu.search_name(key_word,int(offset),length)
        accurate=0
        result=[]
        if len(results)!=0:
            term=results[0]
            if len(term)==2:
                accurate=1
                '''书名+首次出现'''
            result = {"title": term[0],"snippet": add_bold_tag(key_word, term[1])}
        namelist=[]
        if accurate==1:
            namelist=results[1:len(results)-1]
        else:
            namelist=results
        '''人名:相关人物，PR值，首次出现片段'''
        for item in namelist:
            code = {"title": item[0],
                    "displayLink":item[1],
                      "snippet": add_bold_tag(item[0],item[2])
                      }
            codes.append(code)
        ...
        return render_template('index.html', q=key_word, result=result,codes=codes,accurate=accurate,
